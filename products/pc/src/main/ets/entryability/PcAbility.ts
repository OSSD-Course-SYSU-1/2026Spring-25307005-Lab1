...
export default class PcAbility extends UIAbility {
  ...
  onCreate(want: Want, launchParam: AbilityConstant.LaunchParam) {
    hilog.info(0x0000, 'testTag', '%{public}s', 'PcAbility onCreate');
  ...
  onDestroy() {
    hilog.info(0x0000, 'testTag', '%{public}s', 'PcAbility onDestroy');
  }

  onWindowStageCreate(windowStage: window.WindowStage) {
    hilog.info(0x0000, 'testTag', '%{public}s', 'PcAbility onWindowStageCreate');
  ...
  onWindowStageDestroy() {
    hilog.info(0x0000, 'testTag', '%{public}s', 'PcAbility onWindowStageDestroy');
  }

  onForeground() {
    hilog.info(0x0000, 'testTag', '%{public}s', 'PcAbility onForeground');
  }

  onBackground() {
    hilog.info(0x0000, 'testTag', '%{public}s', 'PcAbility onBackground');
  }

  // 迁移时系统自动调用 - 源端保存状态
  async onContinue(wantParam: Record<string, Object>): Promise<AbilityConstant.OnContinueResult> {
    hilog.info(0x0000, 'testTag', '%{public}s', 'PcAbility onContinue');
  ...
  private handleDistributedData(want: Want) {
    hilog.info(0x0000, 'testTag', '%{public}s', 'PcAbility handleDistributedData');
  ...
  onNewWant(want: Want, launchParam: AbilityConstant.LaunchParam) {
    hilog.info(0x0000, 'testTag', '%{public}s', 'PcAbility onNewWant');
